...
class _RevenueChartCardContent extends StatelessWidget {
            ...
            child: Column(
              ...
              children: [
                ...
                if (state.isLoading) ...[
                  ...
                  SizedBox(
                    height: chartHeight,
                    child: _buildChart(state, context, isDesktop),
                  ),
                  SizedBox(height: 2.h),
                  if (state.summary != null)
                    _buildStats(state.summary!, isDesktop),
                ],
                SizedBox(height: 1.h),
      ...
      children: [
        ...
        Container(
          padding: EdgeInsets.symmetric(
            horizontal: isDesktop ? 6.w : 3.w, // Wider on desktop
            vertical: 1.h,
          ),
  ...
  Widget _buildChart(RevenueState state, BuildContext context, bool isDesktop) {
    final revenueData = state.revenueData;
    final screenWidth = MediaQuery.of(context).size.width;
    final maxX = (revenueData.length - 1).toDouble();
    final interval = isDesktop ? 1 : 2; // More labels on wide screens
  ...
  Widget _buildStats(RevenueSummary summary, bool isDesktop) {
    return Row(
      mainAxisAlignment: MainAxisAlignment
          .spaceEvenly, // spaceEvenly for better desktop spread
      children: [
        Expanded(
          // Use Expanded for responsive width distribution
